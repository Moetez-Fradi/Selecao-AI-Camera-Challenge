...
def compute_satisfaction_score(history, frame_w, frame_h, prev_score=None, alpha_smooth=0.6):
    ...
    # image coords: y increases downward -> vertical = [0,1]
    vertical = np.array([0.0, 1.0])
    torso_angle = _angle_between_deg(torso_dir, vertical)
    torso_angle = min(torso_angle, 180 - torso_angle)  # 0..90
    ...
    # stricter upright mapping: excellent <=10deg, bad >=40deg
    if torso_angle <= 10:
        upright_score = 1.0
    elif torso_angle >= 40:
        upright_score = 0.0
...
def compute_face_expression_score(face_landmarks, frame_w, frame_h, prev_score=None, alpha_smooth=0.6):
    ...
    # convert to Nx2 numpy (normalized coords)
    pts = np.array([[p.x, p.y] for p in face_landmarks], dtype=np.float32)
    if pts.shape[0] < 300:  # FaceMesh has 468 points typically, but check
        # still proceed with available points
        pass

    # indices used (MediaPipe FaceMesh typical indices)
    # mouth corners and vertical: 61 (l), 291 (r), top lip 13, bottom lip 14
    # eyes for normalization: 33 (l_outer), 263 (r_outer)
    try:
        lm_l_mouth = pts[61]
        lm_r_mouth = pts[291]
        lm_top_lip = pts[13]
    ...
    # pixel distances
    mouth_w = np.linalg.norm((lm_r_mouth - lm_l_mouth))  # normalized
    mouth_h = np.linalg.norm((lm_bottom_lip - lm_top_lip))
    eye_dist = np.linalg.norm(lm_r_eye - lm_l_eye) + 1e-6

    # normalized metrics
    smile_ratio = mouth_w / eye_dist  # larger -> smile
    mouth_open_ratio = mouth_h / eye_dist

    # map ranges (tweakable)
    smile_score = _map_to_01(smile_ratio, 0.35, 0.75)   # 0.35..0.75 -> 0..1
    open_score = _map_to_01(mouth_open_ratio, 0.02, 0.18)  # 0..1

    # combine: smile weighted higher
...
while True:
    ...
    if best_box is not None:
        ...
        if now - last_print >= PRINT_INTERVAL:
            ...
            # print("  body_diag:", body_diag)
            # print("  face_diag:", face_diag)
