...
model = YOLO("yolov8x-pose.pt")                # Stronger model with pose
...
# Add OpenCV Haar cascade for fallback face detection
...
EYE_CONF_THRESHOLD = 0.5  # Confidence threshold for eyes to determine facing camera

# COCO keypoint connections for drawing pose mesh
COCO_CONNECTIONS = [
...
while True:
    ...
    if not monitoring_mode:
        ...
        if current_candidates:
            ...
            # Check if facing camera using YOLO pose keypoints (both eyes detected with high conf)
            facing_camera = False
            if results[0].keypoints is not None:
                for i in range(len(results[0].boxes)):
                    if results[0].boxes[i].id is not None and int(results[0].boxes[i].id.item()) == best_tid:
                        keypoints = results[0].keypoints.data[i].cpu().numpy()  # (17, 3)
        ...
        if len(transaction_locations) > 10:  # Min points to cluster
            ...
            if len(np.unique(lbls[lbls >= 0])) > 0:
                ...
                avg_w = np.mean([bbox_area(b) for hist in bbox_histories.values() for b in hist if hist]) ** 0.5  # approx width
            ...
            if overlap > OVERLAP_THRESH:
                ...
                if tid == current_client_tid:
                    ...
                    if results[0].keypoints is not None and len(results[0].keypoints) > 0:
                        for i, kp in enumerate(results[0].keypoints):  # Iterate over detected persons
                            if results[0].boxes[i].id is not None and int(results[0].boxes[i].id.item()) == tid:  # Match to current track ID
