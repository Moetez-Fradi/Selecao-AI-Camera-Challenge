...
cap = cv2.VideoCapture("./testcases/swey.mp4")  # replace with "video.mp4" to read local file
...
tracks = {}  # id -> {bbox, first_seen, last_seen, seen_frames, centroid_history, pose_history, face_hits, confirmed, stationary_since}
...
def is_facing_camera(pose_lm, face_det_bbox, person_bbox, fw, fh):
    ...
    if face_det_bbox is not None:
        ...
        bx1,by1,bx2,by2 = person_bbox  # fallback center comparison
...
while True:
    ...
    for tid, tr in tracks.items():
        ...
        # activity from centroid history
        moving = True
    ...
    if confirmed_tracks:
        ...
        if pose_res.pose_landmarks:
            ...
            # reuse compute_satisfaction_score but append to a temporary history
            # for simplicity, reuse a small local history container
            # (user can integrate global history per-track if desired)
            local_hist = collections.deque(maxlen=HISTORY_LEN)
            local_hist.append(pts)
            # call function (may return None if not enough history)
